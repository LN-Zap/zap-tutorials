---
name: Custom Node Connection
menu: Desktop
---

# Connecting to an External LND Node

This tutorial assumes that you already have an [LND node](https://github.com/lightningnetwork/lnd) up and running - either on
another computer or on your local machine.

## Step 1: Configuring LND

In order to connect Zap to your own lnd node you must first ensure that your node is accessible from the computer where you
intend to run Zap. There are several considerations for this:
...
Using LND's `tlsextradomain` setting you can add additional doman names to the TLS certificate. This will allow connecting to LND using a domain name in adddition to or instead of an IP address.
...
After making either of the above changes you must ask LND to regnerate the TLS certificate. You can do this by deleting the old certificate and restarting your node. For example.
...
Zap must be able to connect to your LND node's gRPC interface, which by default listens on `localhost:1009`.
...
Replace `[HOST]:[PORT]` with the interface to listen on for gRPC connections. For example, to accept connections on all external interaces, you would set it like so:

```toml
rpclisten=0.0.0.0:10009
```

You can enter multiple listen address by repeating the directive over multiple lines. For example, to listen only on `localhost` and `1.2.3.4`:
...
If you are running your LND node behind a network router, you may need to set up port forwarding to ensure that incoming trafic to port `10009` is directed to your lnd node.
...
Once installed, use lndconnect to geneate a connection string for your node.

```sh
$ lndconnect -j
```

You may need to pass additional options to the `lndconnect` command in order to ensure that id generate the desired connection string. Common options include:
...
## Step 2: Connecting from Zap

Now that all of the hard work is out of the way, here comes the fun part!

### Create a new wallet in Zap

![connect](../../public/images/screenshots/desktop-connection-type-connect.png)

### Paste in your LND Connect string
...
Clearly the hardest part of all of this is configuring your LND ndoe and ensuring that it is properly accessible. Here are some things to check incase you run into any issues.

### Test your connection

Verify that you can access LND's port gRPC interface from the compauter where you are runnign Zap:
...
You should see your addditional whitelisted IP addresses and/or domain names in the `Subject Alternative Name` section.
