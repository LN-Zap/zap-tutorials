...
```
$ lnd --bitcoin.active --bitcoin.testnet --debuglevel=debug --bitcoin.node=neutrino --neutrino.connect=188.166.148.62:18333 --autopilot.active --rpclisten=0.0.0.0:10009
```
The above line starts `LND` on Bitcoin's testnet, signals for light client mode, connects to a full node hosted at `188.166.148.62:18333`, turns on LND's [autopilot](https://github.com/lightningnetwork/lnd/tree/master/autopilot) feature, and lastly makes sure we are listening for gRPC connections.
